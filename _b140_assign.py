...
분배: 노동 10(dismissal4+wage3+unemployment2+retirement1; 구형식 회수로 wage 슬롯 부활, 산재 0 유지)
      대박7 28(fraud7/sex-crime5/assault4/dui3/divorce4/defamation2/jeonse-fraud3)
      ⚠️ b140: defamation 4→2 축소(lower 잔여 2), 축소분은 fraud+1·sex-crime+1 재배분. 크롤 성공 시 4 복귀.
...
TIER = {  # 도메인별 사용할 판례 tier (pool 가용성 기준)
    ...
    # ⚠️ b140: defamation supreme 완전 소진(b137 97도133 이 마지막) → lower 고정.
    "divorce": "supreme", "unemployment": "lower", "defamation": "lower",
    "wage": "supreme",
}
...
#   다음 배치(b140)부터 defamation 은 TIER 를 "lower" 로 고정하면 된다.
# b140 적용 완료: defamation supreme 0 → TIER 에서 "lower" 고정. TIER_SEQ 는 비움(분기 불필요).
# ⚠️ b140 신규 마름 — jeonse-fraud supreme 잔여 2건인데 슬롯은 3개(prep 실측 supreme=2 / lower=12).
...
#   ⚠️ b141 부터는 jeonse-fraud supreme 0 이 되므로 TIER 에서 "lower" 고정으로 바꿔야 한다.
TIER_SEQ = {"jeonse-fraud": ["supreme", "supreme", "lower"]}
_tier_cur = {}
...
PLAN = [
    ...
    # ── defame-divorce-inherit-traffic (10): defamation2 + fraud1 + sex-crime1 + divorce4 + inheritance1 + traffic1 ──
    # ⚠️ b140 임시 조치 — defamation 4 → 2슬롯 축소.
    ...
    ("defame-divorce-inherit-traffic", "defamation", "accused", "판단형"),
    ("defame-divorce-inherit-traffic", "fraud", "victim", "판단형"),
    ("defame-divorce-inherit-traffic", "sex-crime", "victim", "판단형"),
