# -*- coding: utf-8 -*-
"""batch145 assemble — _b145/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch145-fraud-sexcrime.ts", "spokesBatch145FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch145-assault-dui-jeonsefraud.ts", "spokesBatch145AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch145-defame-divorce-inherit-traffic.ts", "spokesBatch145DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch145-labor.ts", "spokesBatch145Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch145 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-09-10)")
    hdr.append("// 고유 존재 이유:")
...
if "spokesBatch145Labor" not in src:
    ...
    print("spoke-pages.ts already has batch145")
