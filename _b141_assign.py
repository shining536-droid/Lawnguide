...
분배: 노동 10(dismissal4+wage3+unemployment2+retirement1; 구형식 회수로 wage 슬롯 부활, 산재 0 유지)
      ...
      ⚠️ b141: defamation 4→2 축소(lower 잔여 2), 축소분은 fraud+1·sex-crime+1 재배분. 크롤 성공 시 4 복귀.
...
TIER = {  # 도메인별 사용할 판례 tier (pool 가용성 기준)
    ...
    # ⚠️ b141: defamation supreme 완전 소진(b137 97도133 이 마지막) → lower 고정.
    # ⚠️ b141: 명예 민사 크롤 75건 반영으로 defamation supreme 38건 확보 → lower 고정 해제, supreme 복귀.
    #   (b138~b140 은 supreme 0 이라 lower 고정이었음. lower 30건은 예비로 남는다.)
    # ⚠️ b141: jeonse-fraud supreme 0 소진(b140 이 마지막 2건 사용) → lower 고정. TIER_SEQ 는 비운다.
...
#   다음 배치(b141)부터 defamation 은 TIER 를 "lower" 로 고정하면 된다.
# b141 적용 완료: defamation supreme 0 → TIER 에서 "lower" 고정. TIER_SEQ 는 비움(분기 불필요).
# ⚠️ b141 신규 마름 — jeonse-fraud supreme 잔여 2건인데 슬롯은 3개(prep 실측 supreme=2 / lower=12).
...
TIER_SEQ = {}   # b141: jeonse-fraud 는 TIER 에서 lower 고정으로 전환 — 슬롯별 분기 불필요
_tier_cur = {}
...
PLAN = [
    ...
    # ⚠️ b141 임시 조치 — defamation 4 → 2슬롯 축소.
