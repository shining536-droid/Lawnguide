# -*- coding: utf-8 -*-
"""batch139 assemble — _b139/out/*.json → 4 TS files + spoke-pages.ts imports."""
import json, os, re, glob, collections

REPO = r"C:\Users\shini\easylaw-scraper"
BASE = os.path.join(REPO, "_b139")
...
FILES = {
    "fraud-sexcrime": ("spokes-batch139-fraud-sexcrime.ts", "spokesBatch139FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch139-assault-dui-jeonsefraud.ts", "spokesBatch139AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch139-defame-divorce-inherit-traffic.ts", "spokesBatch139DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch139-labor.ts", "spokesBatch139Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch139 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-17)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch138Labor } from './spoke/spokes-batch138-labor';"
assert prev_anchor in src, "batch138 import anchor 없음"
if "spokesBatch139Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch138Labor,\s*$", src, re.M)
    assert m, "batch138 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch139")
