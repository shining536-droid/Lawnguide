# -*- coding: utf-8 -*-
"""batch135 verify — 8 방어선 + 중복/정합성. BASE=_b135/."""
...
for i, d in outs.items():
    ...
    # 구형식 확장(b135~) 대응 재설계: 2~4자리 연도부는 이제 정상 사건번호라 위 bodynums 가 FAIL 로 잡는다.
...
for i, d in outs.items():
    for k in prev_kw:
        ...
        if len(ov) >= 4:
            warns.append(f"[{i:02d}] b134 키워드 교차 {sorted(ov)} vs '{k[:30]}'")
