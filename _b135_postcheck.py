# -*- coding: utf-8 -*-
"""batch135 사후검증 — internalLink 실존 / 판례-주제 정합성(오도메인) / 재결 크로스배치 / 슬러그 총량."""
import json, os, re, glob, collections, io, sys

REPO = r"C:\Users\shini\easylaw-scraper"
SPOKE = os.path.join(REPO, "web", "src", "data", "spoke")
args = {a["idx"]: a for a in json.load(io.open(os.path.join(REPO, "_b135", "args.json"), encoding="utf-8"))}

# ── 전체 슬러그 (batch135 포함 최종 상태) ─────────────────────────────
...
b135_files = glob.glob(os.path.join(SPOKE, "spokes-batch135-*.ts"))
hrefs = []
for f in b135_files:
...
MANUAL_XDOM = {"2000도1731": "dui"}   # traffic 형사 폴백(의도된 크로스도메인)
cache = {}
...
for a in args.values():
    b135_jae |= set(JAE.findall(a["case"]["num"]))
prev_jae = set()
for f in glob.glob(os.path.join(SPOKE, "*.ts")):
    if "batch135" in os.path.basename(f):
        continue
    prev_jae |= set(JAE.findall(io.open(f, encoding="utf-8").read()))
reuse = b135_jae & prev_jae
if reuse:
...
print(f"판례 오도메인: {len(xdom)} (의도된 traffic 형사폴백 1건 제외)")
print(f"재결 b135 사용 {sorted(b135_jae)} | 크로스배치 재사용 {sorted(reuse)}")
