# -*- coding: utf-8 -*-
"""batch140 assemble — _b140/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch140-fraud-sexcrime.ts", "spokesBatch140FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch140-assault-dui-jeonsefraud.ts", "spokesBatch140AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch140-defame-divorce-inherit-traffic.ts", "spokesBatch140DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch140-labor.ts", "spokesBatch140Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch140 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-18)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch139Labor } from './spoke/spokes-batch139-labor';"
assert prev_anchor in src, "batch139 import anchor 없음"
if "spokesBatch140Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch139Labor,\s*$", src, re.M)
    assert m, "batch139 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch140")
