# -*- coding: utf-8 -*-
"""batch141 assemble — _b141/out/*.json → 4 TS files + spoke-pages.ts imports."""
import json, os, re, glob, collections

REPO = r"C:\Users\shini\easylaw-scraper"
BASE = os.path.join(REPO, "_b141")
...
FILES = {
    "fraud-sexcrime": ("spokes-batch141-fraud-sexcrime.ts", "spokesBatch141FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch141-assault-dui-jeonsefraud.ts", "spokesBatch141AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch141-defame-divorce-inherit-traffic.ts", "spokesBatch141DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch141-labor.ts", "spokesBatch141Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch141 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-20)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch140Labor } from './spoke/spokes-batch140-labor';"
assert prev_anchor in src, "batch140 import anchor 없음"
if "spokesBatch141Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch140Labor,\s*$", src, re.M)
    assert m, "batch140 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch141")
