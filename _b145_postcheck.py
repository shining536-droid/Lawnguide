# -*- coding: utf-8 -*-
"""batch145 사후검증 — internalLink 실존 / 판례-주제 정합성(오도메인) / 재결 크로스배치 / 슬러그 총량."""
import json, os, re, glob, collections, io, sys

REPO = r"C:\Users\shini\easylaw-scraper"
SPOKE = os.path.join(REPO, "web", "src", "data", "spoke")
args = {a["idx"]: a for a in json.load(io.open(os.path.join(REPO, "_b145", "args.json"), encoding="utf-8"))}

# ── 전체 슬러그 (batch145 포함 최종 상태) ─────────────────────────────
...
b145_files = glob.glob(os.path.join(SPOKE, "spokes-batch145-*.ts"))
assert b145_files, "batch145 TS 글롭 0 — gen_ts 선행 필요"
hrefs = []
for f in b145_files:
...
for a in args.values():
    b145_jae |= set(JAE.findall(a["case"]["num"]))
prev_jae = set()
for f in glob.glob(os.path.join(SPOKE, "*.ts")):
    if "batch145" in os.path.basename(f):
        continue
    prev_jae |= set(JAE.findall(io.open(f, encoding="utf-8").read()))
reuse = b145_jae & prev_jae
if reuse:
...
print(f"재결 b145 사용 {sorted(b145_jae)} | 크로스배치 재사용 {sorted(reuse)}")
