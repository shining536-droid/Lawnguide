...
prev_kw = []
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "spokes-batch132-*.ts")):
    src = open(f, encoding="utf-8").read()
...
# (A)+(B) b133: defamation 전용 판시사항 폴백. 요지<400 이어도 판시>=400 & 명예훼손류 signal 있으면 회수.
...
def build_pool(domain, want=12):
    ...
    for r in rows:
        ...
        # 개별 재결번호(제29호)가 이미 쓰였으면 복합 전체를 배제 (b133 사고: 복합 문자열이 used_nums 와 안 맞아 재사용됨)
