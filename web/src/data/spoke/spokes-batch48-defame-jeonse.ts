...
export const spokesBatch48DefameJeonse: SpokePage[] = [
  ...
  // ─── 4. jeonse / deposit-not-returned-move-out-procedure ───
  {
    ...
    perspective: 'victim',
    meta: {
      title: '전세보증금 못 받고 이사 가야 하나요? | 로앤가이드',
      description:
        '계약 종료 후 보증금이 미반환된 상태에서 이사하더라도 임차권등기명령으로 우선변제권 유지가 가능해요. 절차를 순서대로 확인해보세요.',
    },
    intro:
      '<p>계약 종료 후 임대인이 보증금을 안 돌려주는 상황에서 이사를 가야 한다면, 임차권등기명령을 먼저 신청하는 것이 핵심이에요. 등기 완료 후 이사하면 대항력·우선변제권이 그대로 유지되는 영역이라 새 입주자가 들어와도 보호가 유지되는 사례가 일반적입니다. 주택임대차보호법 제3조의3 기준입니다.</p>',
    sections: [
      {
    ...
    internalLinks: [
      ...
      { label: '보증금 반환 지연 월 12% 이자', href: '/guide/jeonse/jeonse-deposit-return-delay-monthly-interest-claim-track' },
