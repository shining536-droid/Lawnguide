...
export const spokesBatch27RetirementWage: SpokePage[] = [
  ...
  // ── 2. 퇴직금 지연이자 20% ──
  {
    ...
    perspective: 'victim',
    meta: {
      title: '퇴직금 지연이자 연 20%, 청구 가능한가요? | 로앤가이드',
      description: '퇴직 후 14일 지나도 미지급이면 연 20% 지연이자 청구가 가능해요. 산정·진정·민사 절차를 순서대로 확인해보세요.',
    },
    intro: '<p>퇴직금은 퇴직일로부터 14일 이내 지급이 원칙이고, 그 기한을 넘기면 연 20% 지연이자가 자동 발생하는 영역이에요(근로기준법 제37조). 지연이자는 본인 청구 의사 표시 없이도 발생하지만 청구·산정 절차가 필요한 사례. 1350 진정 또는 민사 청구 둘 다 가능하고, 시효 3년 안 청구 가능합니다.</p>',
    ...
    cases: [{ title: '판례 — 퇴직금 14일 규정', summary: '근로기준법 제36조는 사용자에게 근로자가 사망 또는 퇴직한 경우 14일 이내에 임금, 보상금, 그 밖의 모든 금품을 지급하도록 의무화하고 있으며, 이를 위반하면 3년 이하의 징역 또는 3,000만원 이하의 벌금에 처할 수 있습니다.', takeaway: '비슷한 상황이라면 14일이 지났다면 지연이자와 함께 청구하세요. 사업주의 형사처벌 사유이기도 합니다.' }],
    faq: [
