# -*- coding: utf-8 -*-
"""batch145 verify — 8 방어선 + 중복/정합성. BASE=_b145/."""
...
CASE_ANY = re.compile(r"\b\d{2,4}(?:다|도|두|스|므|모|르|나|가합|가단|가소|드단|드합|느합|느단|노|고합|고단|고정|구합|브|드|느)\d+\b|20\d{2}재결\s*제?\s*\d+\s*호")
...
for i, d in outs.items():
    ...
    full = set(re.findall(r"\b\d{2,4}(?:다|도|두|스|므|모|르|나|가합|가단|가소|드단|드합|느합|느단|노|고합|고단|고정|구합|브|드|느)\d+\b", d["caseSummary"])) | set(re.findall(r"20\d{2}재결\s*제?\s*\d+\s*호", d["caseSummary"]))
    ...
    bodynums = set(re.findall(r"\b\d{2,4}(?:다|도|두|스|므|모|르|나|가합|가단|가소|드단|드합|느합|느단|노|고합|고단|고정|구합|브|드|느)\d+\b", body_of(d))) | set(re.findall(r"20\d{2}재결\s*제?\s*\d+\s*호", body_of(d)))
    ...
    # b145 추가: 판례 요지의 익명화·소송기록 표기가 본문에 그대로 남는 것을 차단
    mask = [w for w in ("○○", "甲", "乙", "丙", "소외 ", "공소외 ", "피고 1", "피고 2", "원고 1") if w in blob]
    if mask: fails.append(f"{tag} 마스킹·소송기록 표기 잔존: {mask}")
...
for i, d in outs.items():
    for k in prev_kw:
        ...
        if len(ov) >= 4:
            warns.append(f"[{i:02d}] b144 키워드 교차 {sorted(ov)} vs '{k[:30]}'")
