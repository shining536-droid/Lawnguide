# -*- coding: utf-8 -*-
"""batch144 assemble — _b144/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch144-fraud-sexcrime.ts", "spokesBatch144FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch144-assault-dui-jeonsefraud.ts", "spokesBatch144AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch144-defame-divorce-inherit-traffic.ts", "spokesBatch144DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch144-labor.ts", "spokesBatch144Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch144 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-29)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch143Labor } from './spoke/spokes-batch143-labor';"
assert prev_anchor in src, "batch143 import anchor 없음"
if "spokesBatch144Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch143Labor,\s*$", src, re.M)
    assert m, "batch143 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch144")
