# -*- coding: utf-8 -*-
"""batch143 assemble — _b143/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch143-fraud-sexcrime.ts", "spokesBatch143FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch143-assault-dui-jeonsefraud.ts", "spokesBatch143AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch143-defame-divorce-inherit-traffic.ts", "spokesBatch143DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch143-labor.ts", "spokesBatch143Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch143 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-23)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch142Labor } from './spoke/spokes-batch142-labor';"
assert prev_anchor in src, "batch142 import anchor 없음"
if "spokesBatch143Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch142Labor,\s*$", src, re.M)
    assert m, "batch142 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch143")
