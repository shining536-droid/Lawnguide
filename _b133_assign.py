...
TIER = {  # 도메인별 사용할 판례 tier (pool 가용성 기준)
    ...
    # ⚠️ b134 이후 값. b133 실제 실행 시에는 divorce="lower"(가용 1건), defamation="supreme"(판시폴백 4건) 이었음.
    # 구형식 정규식 확장 후 divorce supreme 35건 확보 → supreme 복귀(정상 4슬롯 가능).
    # defamation 은 b133 에서 supreme 4건 전량 소비 → 확장으로 27건 재확보되어 supreme 유지.
...
PLAN = [
    ...
    ("fraud-sexcrime", "sex-crime", "accused", "판단형"),
    # b133 재배분(divorce 판례 고갈 4→1): fraud+1 / sex-crime+1 / assault+1
    ("fraud-sexcrime", "fraud", "victim", "절차형"),
    ("fraud-sexcrime", "sex-crime", "victim", "절차형"),
    # ── assault-dui-jeonsefraud (10) ──
    ...
    ("assault-dui-jeonsefraud", "jeonse-fraud", "victim", "판단형"),
    ("assault-dui-jeonsefraud", "jeonse-fraud", "victim", "판단형"),
    ("assault-dui-jeonsefraud", "assault", "neutral", "절차형"),
    # ── defame-divorce-inherit-traffic (10) ──
...
# ── b133 수동 판례 라우팅 (자동 선정 우회) ──────────────────────────
# traffic 형사책임 슬롯: 도주차량 판례(richness 하위라 자동 미선정) — b133~137 순차 6건 중 1번
TRAFFIC_MANUAL = ("dui", "2005도9743")   # kb/dui/cases.json 에 실존, 미사용 (b133 순번 2/6)
NOTES = {
    "2005도9743": "형사책임(특가법 도주차량 제5조의3 제1항 + 형법 제268조) 각도. "
                  "공소장 기재·적용법조 오류와 불고불리 원칙, 사망(제1호)/상해(제2호) 구분이 쟁점. "
                  "형사처벌 단정 금지(‘처벌됩니다’ 류 금지), accused 안전구문 ‘혐의를 받고 있다면’ 포함. "
                  "보험금 민사 각도와 구분 — 형사 도주죄 의율·심판범위 중심.",
    "2010도1444": "판시[3](정보통신망 명예훼손·군무원 허위사실 게시)만 근거로 사용. 판시[1][2]는 의료법이라 배제.",
}
