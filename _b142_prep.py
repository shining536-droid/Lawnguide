...
prev_kw = []
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "spokes-batch141-*.ts")):
    src = open(f, encoding="utf-8").read()
...
LOWER_SUF = {
    ...
    # b142 확장(①): 형사 접미사만 있어 민사 하급심(명예훼손 손해배상)이 풀에 진입조차 못 하던 문제 해소.
    ...
    # ⚠️ b142 신설: dui 는 LOWER_SUF 키 자체가 없어 low_re=None → lower 후보를 한 건도 검사하지 않고
...
DEF_NAME_CORE = ["명예훼손", "모욕", "비방", "출판물"]   # b142 ②: 사건명만으로 주제 확인되는 건 → BLOB 게이트 면제
DEF_CIVIL_SUF = re.compile(r"^\d{2,4}(?:다|나|가합|가단|가소)\d+$")   # b142 ②: defamation 민사 판별
...
NAME_INCLUDE = {
    ...
    # ⚠️ b142 ①: 명예 민사 크롤 유입분 대응. 민사 판례는 사건명이 죄다 '손해배상(기)'·'손해배상(국)' 계열이라
...
BLOB_INCLUDE = {
    # ⚠️ b142 ②: defamation 민사 순도 게이트. NAME 완화(①)로 들어오는 오염을 본문 신호로 잘라낸다.
...
def build_pool(domain, want=12):
    ...
    for r in rows:
        ...
        if binc:
            # ⚠️ b142 ② 적용범위 한정: defamation 은 **민사 유입분에만** BLOB 게이트를 건다.
