# -*- coding: utf-8 -*-
"""batch135 assemble — _b135/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch135-fraud-sexcrime.ts", "spokesBatch135FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch135-assault-dui-jeonsefraud.ts", "spokesBatch135AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch135-defame-divorce-inherit-traffic.ts", "spokesBatch135DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch135-labor.ts", "spokesBatch135Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch135 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-04)")
    hdr.append("// 고유 존재 이유:")
...
last126 = "import { spokesBatch134Labor } from './spoke/spokes-batch134-labor';"
assert last126 in src, "batch130 import anchor 없음"
if "spokesBatch135Labor" not in src:
    src = src.replace(last126, last126 + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch134Labor,\s*$", src, re.M)
    assert m, "batch130 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch135")
