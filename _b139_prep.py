...
prev_kw = []
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "spokes-batch138-*.ts")):
    src = open(f, encoding="utf-8").read()
...
LOWER_SUF = {
    ...
    # b139 확장(①): 형사 접미사만 있어 민사 하급심(명예훼손 손해배상)이 풀에 진입조차 못 하던 문제 해소.
...
BLOB_INCLUDE_MIN = {"traffic-accident": 2}
...
def build_pool(domain, want=12):
    ...
    for r in rows:
        ...
        if binc and sum(1 for w in binc if w in blob) < BLOB_INCLUDE_MIN.get(domain, 1):
            continue
