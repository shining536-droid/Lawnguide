...
prev_kw = []
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "spokes-batch140-*.ts")):
    src = open(f, encoding="utf-8").read()
...
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "*.ts")):
    src = open(f, encoding="utf-8").read()
    # ②안 구형식 확장: 한국 구판례는 '96므1417' 처럼 2자리 연도. \d{2,4} 로 신·구형식 동시 수집.
...
LOWER_SUF = {
    ...
    # b141 확장(①): 형사 접미사만 있어 민사 하급심(명예훼손 손해배상)이 풀에 진입조차 못 하던 문제 해소.
...
DEF_NAME_CORE = ["명예훼손", "모욕", "비방", "출판물"]   # b141 ②: 사건명만으로 주제 확인되는 건 → BLOB 게이트 면제
DEF_CIVIL_SUF = re.compile(r"^\d{2,4}(?:다|나|가합|가단|가소)\d+$")   # b141 ②: defamation 민사 판별
...
NAME_INCLUDE = {
    ...
    # ⚠️ b141 ①: 명예 민사 크롤 유입분 대응. 민사 판례는 사건명이 죄다 '손해배상(기)'·'손해배상(국)' 계열이라
...
BLOB_INCLUDE = {
    # ⚠️ b141 ②: defamation 민사 순도 게이트. NAME 완화(①)로 들어오는 오염을 본문 신호로 잘라낸다.
...
def build_pool(domain, want=12):
    ...
    seen_nums = set()
    dedup_dropped = []
    # 구형식(2자리 연도) 포함 스캔. (19|20)\d{2} 로는 '96므1417' 이 안 잡히므로 \d{2,4}.
    ...
    for r in rows:
        ...
        if binc:
            # ⚠️ b141 ② 적용범위 한정: defamation 은 **민사 유입분에만** BLOB 게이트를 건다.
