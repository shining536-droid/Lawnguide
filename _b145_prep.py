...
prev_kw = []
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "spokes-batch144-*.ts")):
    src = open(f, encoding="utf-8").read()
...
for f in glob.glob(os.path.join(REPO, "web", "src", "data", "spoke", "*.ts")):
    ...
    # ⚠️ b145 수정: b144 가 LOWER_SUF[divorce] 에 드/느/느단/브 를 추가하고도 여기를 안 고쳐
    #   96드96056·97느1666 이 used 미기록으로 남아 다음 배치에 재출현할 상태였다. 세트로 확장함.
    # ⚠️ picker 확장과 반드시 세트 — 여기가 빠지면 기존 스포크에 이미 인용된 구형식이 used 미기록으로 재출현함.
    used_nums |= set(re.findall(r"\b\d{2,4}(?:도|두|다|므|스|르|나|가합|가단|가소|드단|드합|느합|느단|노|고합|고단|고정|구합|브|드|느)\d+\b", src))
...
LOWER_SUF = {
    # ⚠️ b144 후속: 가사 사건부호 누락분 확장(드·브·느·느단). 즈단·즈합·너는 raw 0건이라 제외.
    #   ⚠️ 교대 순서는 긴 것 우선 — `드`가 `드단`보다 앞이면 $ 앵커에서 조기매칭 실패한다.
    #   ⚠️ DIVORCE_FAMILY_SUF(2차 관문)도 반드시 같이 고칠 것. 한쪽만 고치면 순증 0.
    "divorce": r"(?:드단|드합|느합|느단|르|드|느|브)",
    "wage": r"(?:나|가합|가단)",
...
DIVORCE_FAMILY_SUF = re.compile(r"^\d{2,4}(?:므|드단|드합|느합|느단|르|드|느|브)\d+$")   # b144 후속: LOWER_SUF 와 세트 확장
