...
분배: 노동 10(dismissal4+wage3+unemployment2+retirement1; 구형식 회수로 wage 슬롯 부활, 산재 0 유지)
      대박7 28(fraud6/defamation5/assault5/sex-crime4/dui3/jeonse-fraud3/divorce2)
      ⚠️ b143: divorce 4→2 축소(lower 7건 전량 구법), 축소분은 assault+1·defamation+1 재배분.
...
TIER = {  # 도메인별 사용할 판례 tier (pool 가용성 기준)
    "fraud": "supreme", "sex-crime": "supreme", "assault": "supreme",
    # ⚠️ b143: dui supreme 0 소진(b142 가 마지막 1건 사용) → lower 고정. lower 25건 보유.
    "dui": "lower",
    "inheritance": "supreme",
    ...
    # ⚠️ b143: defamation supreme 완전 소진(b137 97도133 이 마지막) → lower 고정.
    # ⚠️ b143: 명예 민사 크롤 75건 반영으로 defamation supreme 38건 확보 → lower 고정 해제, supreme 복귀.
    #   (b138~b140 은 supreme 0 이라 lower 고정이었음. lower 30건은 예비로 남는다.)
    # ⚠️ b143: jeonse-fraud supreme 0 소진(b140 이 마지막 2건 사용) → lower 고정. TIER_SEQ 는 비운다.
    # ⚠️ b143: divorce supreme 0 소진(b142 가 마지막 3건 사용) → lower 고정. lower 잔여 7건 전량 구법(1985~2000).
    "divorce": "lower", "unemployment": "lower", "defamation": "supreme",
    "jeonse-fraud": "lower",
    "wage": "supreme",
}
...
#   다음 배치(b143)부터 defamation 은 TIER 를 "lower" 로 고정하면 된다.
# b143 적용 완료: defamation supreme 0 → TIER 에서 "lower" 고정. TIER_SEQ 는 비움(분기 불필요).
# ⚠️ b143 신규 마름 — jeonse-fraud supreme 잔여 2건인데 슬롯은 3개(prep 실측 supreme=2 / lower=12).
...
#   ⚠️ b143 부터는 jeonse-fraud supreme 0 이 되므로 TIER 에서 "lower" 고정으로 바꿔야 한다.
# ⚠️ b143: divorce supreme 잔여 3 인데 슬롯 4 → 4번째만 lower(8건 보유)로 분기.
...
# ⚠️ b143: divorce·dui 모두 supreme 0 → TIER 에서 lower 고정으로 전환. TIER_SEQ 는 비운다.
#   (TIER_SEQ 에 supreme 이 남아 있으면 pick_case 가 빈 pool 을 요구해 SystemExit)
TIER_SEQ = {}   # jeonse-fraud·defamation·divorce·dui 는 TIER 에서 직접 고정
_tier_cur = {}
...
PLAN = [
    ...
    # ── defame-divorce-inherit-traffic (10): defamation5 + divorce2 + assault1 + inheritance1 + traffic1 ──
    # ⚠️ b143 divorce 슬롯 축소 4 → 2.
    #   사유: divorce supreme 0, lower 잔여 7건이 전량 구법(1985~2000). 4슬롯이면 b144 에서 소진·중단된다.
    #   2슬롯이면 3배치분(7 → 5 → 3 → 1) 확보 → 크롤/진단 시간을 번다.
    #   남는 2슬롯은 pool 여유가 있는 assault(+1) · defamation(+1) 로 재배분해 대박7 비중 70% 를 유지한다.
    #   assault 5번째 슬롯만 이 파일그룹에 둔다 — 에이전트 idx 범위를 10개씩 고정하기 위함.
    #   (파일명은 라벨일 뿐이고 실제 구성은 TS 헤더에 자동 기재됨)
    ("defame-divorce-inherit-traffic", "defamation", "accused", "판단형"),
    ("defame-divorce-inherit-traffic", "defamation", "accused", "판단형"),
    ("defame-divorce-inherit-traffic", "defamation", "victim", "판단형"),
    ("defame-divorce-inherit-traffic", "defamation", "victim", "판단형"),
    ("defame-divorce-inherit-traffic", "defamation", "neutral", "절차형"),
    ("defame-divorce-inherit-traffic", "divorce", "neutral", "판단형"),
    ("defame-divorce-inherit-traffic", "divorce", "victim", "절차형"),
    ("defame-divorce-inherit-traffic", "assault", "victim", "판단형"),
...
# ── b145 정합성 사전검수(육안) 결과 수동 교체 ─────────────────────────
#   richness 자동픽 상위에 '주죄명은 맞지만 판시 실체가 오도메인/절차법리'인 건이 다수 올라왔다.
#   (예: fraud 2006도4418=약사법 조제, 2017도20616=금융지배구조법 경합범 분리심리,
#        assault 87도2297=보호감호 청구변경, 2019도16484=사드기지 주거침입,
#        jeonse-fraud 2012나60123=리조트 회원보증금(회사정리법), sex-crime 2006도7228=형소법 314조)
#   FORCE[idx] = 사건번호 — 해당 도메인/tier pool 안에서만 고른다(풀 우회 아님, used/blacklist 게이트 통과분).
FORCE = {
    # wage: 풀 상위가 전부 '퇴직금' 사건이라 임금 도메인 각도 확보용 교체
    4: "94다26615",    # 체불임금 — 통상임금 정의, 가족수당·근속수당 포함 여부
    5: "91다21381",    # 위탁직 시청료징수원 근로자성(근로기준법상 근로자)
    6: "95다55733",    # 평균임금 = 사용자 관리·지배 가능 부분 한정(택시 사납금 초과수입)
    # fraud: 약사법/금융지배구조법/정보통신망법 판시 3건 교체
    12: "2011도4440",  # 컴퓨터등사용사기 — '부정한 명령의 입력'
    13: "95도1874",    # 소송사기 객체
    14: "2004도6557",  # 사기·사기방조(허위 입원)
    15: "2006도3126",  # 타인 명의 신용카드 현금대출 죄책
    # sex-crime: 형소법 314조 증거능력 판시 교체
    18: "2006도5360",  # 충동조절장애 성격적 결함과 심신장애
    19: "2013도4279",  # 카메라등이용촬영 — 화상채팅 화면은 '사람의 신체'가 아님
    # assault: 보호감호·공갈·주거침입·외국수사기관 증거능력 4건 교체
    20: "94도1484",    # 폭행치사 — 안수기도 유형력 행사가 폭행
    21: "94도2422",    # 피해자의 일관된 진술을 함부로 배척 금지
    22: "99도3377",    # 쌍방 격투 시 유형력 행사의 위법성 조각 요건
    23: "2001도300",   # 현행범 요건 흠결 체포에 대한 반항
    37: "2012도1895",  # 동일 피해자 폭행이 업무방해 수단인 경우의 죄수
    # dui: 구법(도로교통법 제41조, 1992년 1심) 교체
    26: "2008노4888",  # 위드마크 공식 산정 시 주의점
    # jeonse-fraud: 리조트 회원보증금·국제사법 근질권 2건 교체
    28: "2011나79144",   # 소유권 이전 후에도 임대인 지위 존속 여부
    29: "2009가합41675", # 영업양수인의 임대차보증금 반환책임 승계
    # defamation: 언론·정치·종교 편중 완화 + 개인/법인 각도 확보
    30: "2003다66806",   # 인터넷 공개정보만 믿고 적시 — '상당한 이유' 부정
    32: "2021다250735",  # 법인의 사회적 명성·신용 훼손
    33: "2013다34013",   # 위법성 조각 — '오로지 공공의 이익'과 '진실한 사실'의 의미
    34: "2022다222898",  # 의견 표명의 한계와 인격권 침해
    # divorce: 섭외이혼 관할·준거법 → 민법 제840조 정면 쟁점으로 교체
    35: "96드96056",   # 부정행위 용서 후 재발 시 파탄책임(쌍방유책)
    36: "97느1666",    # 협의이혼 전제 재산분할 약정의 효력·산정 기준시
    # inheritance: 연부연납 가산세·전심절차 → 상속포기 각도로 교체
    38: "2004두10289", # 사전증여 받고 상속포기한 자의 상속세 납세의무
    # traffic-accident: 관용차 기명피보험자(국가) → 피해자 직접청구권으로 교체
    39: "97다17544",   # 상법 제724조 제2항 피해자 직접청구권의 법적 성질
}


def forced_case(dom, tier, num):
    for c in pools[dom][tier]:
        if c["num"] == num:
            return c
    raise SystemExit(f"FORCE 판례 {num} 이 {dom}/{tier} pool 에 없음")


args = []
batch_used |= set(FORCE.values())   # 자동픽이 FORCE 대상 판례를 먼저 집어가지 않도록 선점
for i, (fg, dom, persp, typ) in enumerate(PLAN):
    tier = tier_for(dom)
    if i in FORCE:
        c = forced_case(dom, tier, FORCE[i])   # b145 정합성 교체
    elif dom == "traffic-accident" and TRAFFIC_MANUAL:
