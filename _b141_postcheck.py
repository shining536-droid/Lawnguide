# -*- coding: utf-8 -*-
"""batch141 사후검증 — internalLink 실존 / 판례-주제 정합성(오도메인) / 재결 크로스배치 / 슬러그 총량."""
import json, os, re, glob, collections, io, sys

REPO = r"C:\Users\shini\easylaw-scraper"
SPOKE = os.path.join(REPO, "web", "src", "data", "spoke")
args = {a["idx"]: a for a in json.load(io.open(os.path.join(REPO, "_b141", "args.json"), encoding="utf-8"))}

# ── 전체 슬러그 (batch141 포함 최종 상태) ─────────────────────────────
...
b141_files = glob.glob(os.path.join(SPOKE, "spokes-batch141-*.ts"))
assert b141_files, "batch141 TS 글롭 0 — gen_ts 선행 필요"
hrefs = []
for f in b141_files:
...
MANUAL_XDOM = {}   # b141: traffic 형사 시퀀스 6/6 종료 → 수동 크로스도메인 없음. richness 자동픽 복귀.
cache = {}
...
for a in args.values():
    b141_jae |= set(JAE.findall(a["case"]["num"]))
prev_jae = set()
for f in glob.glob(os.path.join(SPOKE, "*.ts")):
    if "batch141" in os.path.basename(f):
        continue
    prev_jae |= set(JAE.findall(io.open(f, encoding="utf-8").read()))
reuse = b141_jae & prev_jae
if reuse:
...
print(f"재결 b141 사용 {sorted(b141_jae)} | 크로스배치 재사용 {sorted(reuse)}")
