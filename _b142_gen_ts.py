# -*- coding: utf-8 -*-
"""batch142 assemble — _b142/out/*.json → 4 TS files + spoke-pages.ts imports."""
...
FILES = {
    "fraud-sexcrime": ("spokes-batch142-fraud-sexcrime.ts", "spokesBatch142FraudSexCrime"),
    "assault-dui-jeonsefraud": ("spokes-batch142-assault-dui-jeonsefraud.ts", "spokesBatch142AssaultDuiJeonseFraud"),
    "defame-divorce-inherit-traffic": ("spokes-batch142-defame-divorce-inherit-traffic.ts", "spokesBatch142DefameDivorceInheritTraffic"),
    "labor": ("spokes-batch142-labor.ts", "spokesBatch142Labor"),
}
...
for fkey, (fname, varname) in FILES.items():
    ...
    hdr.append("// batch142 " + "+".join(f"{k}({v})" for k, v in doms.items()) + f" — {len(items)}개 (2026-08-21)")
    hdr.append("// 고유 존재 이유:")
...
prev_anchor = "import { spokesBatch141Labor } from './spoke/spokes-batch141-labor';"
assert prev_anchor in src, "batch141 import anchor 없음"
if "spokesBatch142Labor" not in src:
    src = src.replace(prev_anchor, prev_anchor + "\n" + imports, 1)
    m = re.search(r"^(\s*)\.\.\.spokesBatch141Labor,\s*$", src, re.M)
    assert m, "batch141 spread anchor 없음"
    ind = m.group(1)
    ...
    print("spoke-pages.ts already has batch142")
